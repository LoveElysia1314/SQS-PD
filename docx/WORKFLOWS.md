...
- `src/sqs_pd/interface/api.py::generate_sqs`
- `src/sqs_pd/runtime/sqs_orchestrator.py::generate_sqs`
...
- `src/sqs_pd/batching/batch_analysis.py`
...
模板变量：
- `generate_sqs_with_report(..., output_folder_name=...)`：支持 `{stem}`、`{supercell}`
- `run_topk_sqs_with_model(..., output_folder_name=...)`：支持 `{stem}`、`{top_k}`

---

## 常见误用

- 误把 dry-run 当作优化结果：dry-run 不返回优化结构。
- 手动改动 split 配置中的占位符语义：容易破坏子晶格映射。
- 批处理续算时改变去重键定义：会导致重复计算或跳过错误。
